...
for n = 1:numel(group_1)
    ...
    if length(x)>=MINIMUM_DATASET_LENGTH
        status(n) = 1;  
        hrv_result = sad.HRV.time_domain_analysis(params, x(1:MINIMUM_DATASET_LENGTH), group_1{n}, 0, 0);
...
for n = 1:numel(group_2)
    ...
    if length(x)>=MINIMUM_DATASET_LENGTH
        status(n) = 1;
        hrv_result = sad.HRV.time_domain_analysis(params, x(1:MINIMUM_DATASET_LENGTH), group_2{n}, 0, 0);
        rtable = struct2table(hrv_result);
        rtable(:,'RR_interval') = [];
